...
static void print_level(int level) {
  if (color == LOG_COLOR_ENABLE) {
    switch (level) {
    case LOG_TRACE:
      fprintf(output, "[%s%s%s] ", level_color[LOG_TRACE],
              level_string[LOG_TRACE], RESET);
      return;
    case LOG_INFO:
      fprintf(output, "[%s%s%s] ", level_color[LOG_INFO],
              level_string[LOG_INFO], RESET);
      return;
    case LOG_DEBUG:
      fprintf(output, "[%s%s%s] ", level_color[LOG_DEBUG],
              level_string[LOG_DEBUG], RESET);
      return;
    case LOG_WARN:
      fprintf(output, "[%s%s%s] ", level_color[LOG_WARN],
              level_string[LOG_WARN], RESET);
      return;
    case LOG_ERROR:
      fprintf(output, "[%s%s%s] ", level_color[LOG_ERROR],
              level_string[LOG_ERROR], RESET);
      return;
    case LOG_FATAL:
      fprintf(output, "[%s%s%s] ", level_color[LOG_FATAL],
  ...
  switch (level) {
  case LOG_TRACE:
    fprintf(output, "[%s] ", level_string[LOG_TRACE]);
    return;
  case LOG_INFO:
    fprintf(output, "[%s] ", level_string[LOG_INFO]);
    return;
  case LOG_DEBUG:
    fprintf(output, "[%s] ", level_string[LOG_DEBUG]);
    return;
  case LOG_WARN:
    fprintf(output, "[%s] ", level_string[LOG_WARN]);
    return;
  case LOG_ERROR:
    fprintf(output, "[%s] ", level_string[LOG_ERROR]);
    return;
  case LOG_FATAL:
    fprintf(output, "[%s] ", level_string[LOG_FATAL]);
    return;
  }
}
